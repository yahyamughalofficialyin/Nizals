...
<body>
  ...
  <div class="image-profile-container">
    <div class="profile-cover-photo-container">
      <img src="<?php echo $_SESSION['dp'] ?>" alt="" class="profile-cover-photo">
    </div>
    <div class="profile-photo-container">
      <img src="<?php echo $_SESSION['dp'] ?>" alt="" class="profile-photo">
    </div>
  </div>
  <div class="title-main">
    <div class="title">
      ...
      <!-- <div class="title-account-options">
        <div class="title-account-options-form" id="follow">
          <button id="followup"><i class='bx bxs-user-plus'></i>
            <p>Follow Up</p>
          </button>
          <button id="followed" hidden><i class='bx bxs-user-check'></i>
            <p>Followed</p>
          </button>
        </div>
        <a href="message.php?user_id=1"><button class="account-message"><i class='bx bxs-message-rounded'></i>
            <p>Message</p>
          </button></a>
      </div> -->
    </div>
  ...
  $query  = "SELECT * FROM `post` WHERE `cl_id` = '{$_SESSION['c_id']}' AND `type` = 'video' ";
  $result = mysqli_query($conn, $query);

  while ($row = mysqli_fetch_assoc($result)) {
  ?>
        <a href="video-post.php?video_id=<?php echo $row['post_id']?>"><video src="admin/upload/post/video/<?php echo $row['post']; ?>"></video></a>
...
<?php } ?>
  ...
  $query  = "SELECT * FROM `post` WHERE `cl_id` = '{$_SESSION['c_id']}' AND `type` = 'picture' ";
  $result = mysqli_query($conn, $query);

  while ($row = mysqli_fetch_assoc($result)) {
  ?>
        <a href=""><img src="admin/upload/post/image/<?php echo $row['post']; ?>" alt=""></a>
        <?php } ?>
  ...
  while ($row = mysqli_fetch_assoc($result)) {
  ?>
   <div class="account-video-post" data-post-id="<?php echo $row['post_id']; ?>">
...
<p>likes count</p>
  ...
  $query  = "SELECT * FROM `post` WHERE `cl_id` = '{$_SESSION['c_id']}' AND `type` = 'picture' ORDER BY `post_id` DESC";
...
<script>
  $(document).ready(function () {
    ...
    $('.account-video-post').on('click', '.video-like-btn', function () {
      var postId = $(this).closest('.account-video-post').data('post-id');
      likePost(postId);
    });

    // Handle unlike button click
    $('.account-video-post').on('click', '.video-liked-btn', function () {
      var postId = $(this).closest('.account-video-post').data('post-id');
      unlikePost(postId);
    });
    ...
    function likePost(postId) {
      $.ajax({
        type: 'POST',
        url: 'like_post.php', // Create a PHP script for liking a post
        data: {
          post_id: postId,
        },
        success: function (response) {
          if (response.success) {
            // Update UI (e.g., change button styles)
            // Update likes count
          } else {
    ...
    function unlikePost(postId) {
      $.ajax({
        type: 'POST',
        url: 'unlike_post.php', // Create a PHP script for unliking a post
        data: {
          post_id: postId,
        },
        success: function (response) {
          if (response.success) {
            ...
            console.error('Unlike failed: ' + response.error);
          }
        },
        error: function () {
